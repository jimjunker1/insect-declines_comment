...
#' @param 
trend_detect <- function(data, cols_keep = NULL,...){
  if(!is.data.frame(data)) data <- data.frame(data)
  
  if(is.null(cols_keep)){data_mod = data[c("Year","Number")]} else{data_mod = data[c(cols_keep, "Year","Number")]}
  trend_summ <- summary(lm(Number~Year, data = data_mod))$coefficients
  df_add <- data.frame(trend = NA, coef = trend_summ['Year','Estimate'], coef_se = trend_summ['Year','Std. Error'])
  if(trend_summ['Year','Pr(>|t|)'] >= 0.05) {
    df_add[,'trend'] <- "no change"
  } else if(trend_summ['Year','Estimate'] > 0) {
    df_add[,'trend'] <- "positive"
  } else {df_add[,'trend'] <- "negative"}
  if(is.null(cols_keep)){return(df_add)} else{cbind(unique(data_mod[,names(data_mod) %in% cols_keep]), df_add)}
}
